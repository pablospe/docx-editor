...
- **Exit code 0 is not always "it ran".** If a previous command *raised* while your `exec` was queued behind it, the kernel discards your request without executing it: you get exit 0, no output, and a `Warning: the kernel discarded this request without running it` line on stderr (`"started": false` in `eval`'s envelope). Nothing of yours happened — re-send it. Read stderr on exit 0, or check `started`, before believing an edit applied.
