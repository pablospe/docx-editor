...
**Replace granularity:** `replace()` trims words shared by `find` and `replace_with` at either end, so only the changed words are written as revisions — a replace that only adds or only removes words becomes a pure insertion or deletion. The insertion carries the formatting of the run that contributed the most characters to the replaced span (ties → earliest run). Replacing text with itself is a **no-op**: no revisions are written and the returned `EditResult` equals the input ref with `group_id=None` and `revision_ids=()` — check that triple to detect it.
