...
The per-id half of #75, which fixed only `accept_all`/`reject_all`. With B's `<w:ins w:id="7">` before A's `<w:del w:id="7">`, `list_revisions(author="A")` returns one row with id 7, and `reject_revision(7)` deletes B's inserted text and leaves A's deletion pending; `accept_revision(7)` makes B's insertion permanent. The caller cannot disambiguate — `Revision` carries no element handle — and `SKILL.md` documents this exact per-id pattern. Fix: either give the per-id verbs an author/identity selector, or refuse an ambiguous id with a teaching error; the manager already knows which ids are ambiguous, since `_reconstruct_groups` is the sole writer of `self._revision_groups[rev_id] = None` (`registry.py`) and writes it only for duplicated ids. SKILL.md carries an interim caveat. Found by the multi-review of PR #96.
